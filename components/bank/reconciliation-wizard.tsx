...
type LedgerEntry = Awaited<ReturnType<typeof getUnreconciledEntries>>[number];
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select the entries that appear on your physical bank statement to
            match the closing balance of ₹
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-muted p-4 rounded-lg">
              ...
              <p className="text-xl font-semibold">
                ₹{(setupData?.openingBalanceBank || 0).toLocaleString("en-IN")}
              </p>
            </div>
            <div className="bg-primary/10 p-4 rounded-lg border border-primary/20">
              <p className="text-sm text-primary">Calculated Closing</p>
              <p className="text-xl font-bold text-primary">
                ₹{calculatedClosing.toLocaleString("en-IN")}
              </p>
            </div>
            <div
              className={cn(
              ...
              <TableBody>
                {loadingEntries ? (
                  ...
                  <TableRow>
                    ...
                    >
                      No unreconciled entries found for this period.
                    </TableCell>
                  </TableRow>
                ) : (
                  entries.map((entry) => {
                    const isDebit = ["EXPENSE", "TRANSFER_OUT"].includes(
                      entry.type,
                    );
                    return (
                      <TableRow
                        key={entry.id}
                        className={
                          matchedIds.has(entry.id) ? "bg-muted/50" : ""
                        }
                      >
                        <TableCell className="text-center">
                          <Checkbox
                            checked={matchedIds.has(entry.id)}
                            onCheckedChange={() => toggleMatch(entry.id)}
                          />
                        </TableCell>
                        <TableCell>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Enter the details from your physical or digital bank statement.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form
