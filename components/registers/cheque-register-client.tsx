...
export function ChequeRegisterClient({
  ...
  const totalReceived = cheques
    .filter((c) => c.nature === "RECEIVED")
    .reduce((sum, c) => sum + c.amount, 0);
  const totalIssued = cheques
    .filter((c) => c.nature === "ISSUED")
    .reduce((sum, c) => sum + c.amount, 0);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Add Cheque Book</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-5">
          ...
          <div className="space-y-2">
            ...
            <Input
              value={form.bookNo}
              onChange={(e) =>
                setForm((f) => ({ ...f, bookNo: e.target.value }))
              }
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, endNumber: e.target.value }))
              }
            />
          </div>
          <div className="flex items-end">
            <Button onClick={submitChequeBook} disabled={isPending}>
              Save
            </Button>
          </div>
        </CardContent>
              ...
              {cheques.map((cheque) => (
                <TableRow key={cheque.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        cheque.nature === "RECEIVED" ? "default" : "secondary"
                      }
                    >
                      {cheque.nature}
                    </Badge>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Cheque Book Leaves</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
